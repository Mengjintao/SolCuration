...
try:
    from .descriptors import rdDescriptors, rdNormalizedDescriptors
...
@register_features_generator('mordred_2d_normalized')
def mordred_features_generator(mol):
    # calc = Calculator(descriptors)
    # features = calc(mol).fill_missing(0).asdict()
    # for name, value in features.items():
    #     features[name] = cdfs_mordred[name](value)
    # features = np.array(list(features.values()))
    # return features
    pass


@register_features_generator('padel_2d_normalized')
def padel_features_generator(mol):
    pass
"""
